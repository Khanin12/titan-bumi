...
import { FileSpreadsheet, FileText, Calendar as CalendarIcon } from 'lucide-react';
...
export default function DriversPage() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const res = await fetch(`/api/drivers?search=${search}&page=${page}&limit=10`);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                fetchData();
                showToast(isEditing ? 'Driver updated successfully' : 'Driver added successfully', isEditing ? 'info' : 'success');
                setCurrentDriver({});
                setIsEditing(false);
            } else {
    ...
    const handleDelete = async () => {
        ...
        try {
            const res = await fetch(`/api/drivers/${currentDriver.id}`, {
                method: 'DELETE',
            });
            if (res.ok) {
                setIsDeleteModalOpen(false);
                fetchData();
                showToast('Driver deleted successfully', 'destructive');
                setCurrentDriver({});
            } else {
                showToast('Failed to delete driver', 'destructive');
            }
        } catch (error) {
        ...
        <div className="p-6 space-y-8">
            <div className="flex flex-col gap-4">
                <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
                    ...
                    <p className="text-muted-foreground">Manage your drivers details.</p>
                </motion.div>
                ...
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-border">
                        {loading ? (
                            ...
                            <tr><td colSpan={4} className="px-6 py-8 text-center">No drivers found.</td></tr>
                        ) : (
                            data.map((item) => (
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-card w-full max-w-md rounded-2xl border border-border shadow-xl overflow-hidden">
                            <div className="p-4 border-b border-border flex justify-between items-center">
                                <h3 className="font-bold text-lg">{isEditing ? 'Edit Driver' : 'Add Driver'}</h3>
                                <button onClick={() => setIsModalOpen(false)}><X className="w-5 h-5" /></button>
                            </div>
                            <form onSubmit={handleSubmit} className="p-4 space-y-4">
                                <div className="space-y-2">
                                    <label className="text-sm font-medium">Name</label>
                                    <input required type="text" value={currentDriver.name || ''} onChange={(e) => setCurrentDriver({ ...currentDriver, name: e.target.value })} className="w-full px-3 py-2 bg-accent/50 border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20" />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-sm font-medium">Phone</label>
                                    <input type="text" value={currentDriver.phone || ''} onChange={(e) => setCurrentDriver({ ...currentDriver, phone: e.target.value })} className="w-full px-3 py-2 bg-accent/50 border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20" />
                                </div>
                ...
                {isDeleteModalOpen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-card w-full max-w-sm rounded-2xl p-6 text-center border border-border shadow-xl">
                            ...
                            <h3 className="font-bold text-lg mb-2">Delete Driver?</h3>
                            <p className="text-muted-foreground text-sm mb-6">Are you sure you want to delete <strong>{currentDriver.name}</strong>?</p>
